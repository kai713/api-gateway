...
public class Routes {
    ...
    public RouterFunction<ServerResponse> productServiceRoute() {
        return GatewayRouterFunctions.route("product_service")
                ...
                //Этот: .filter(circuitBreaker("productServiceCircuitBreaker" как то взаимодействует
                // с application.properties resilience4j (?) и как я понял в качестве первого параметра принимает id string для логов (?)
                // в качестве второго параметра принимается fallbackRoute, которая вызывает функцию fallbackRoute()
